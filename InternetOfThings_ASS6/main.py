...
def GetDeviceIDFromUserID(userID):
  try:
        ...
        url = 'https://europe-west9-simf-403016.cloudfunctions.net/SIMF_UserIDToDeviceID'
        req = urequests.get(url, headers=headers)
        
        if req.status_code == 200:
            JSONdeviceIDResponse = json.loads(req.text)
            deviceID = get_json_key(JSONdeviceIDResponse, 'deviceID')
            ...
            raise ValueError("Request failed with status code: " + str(req.status_code))
  except:
    screen.clean_screen
    hideAll()
...
def getLogsFromDeviceID(deviceID):
  try:
        headers = {"deviceID": str(deviceID)}
        url = 'https://europe-west9-simf-403016.cloudfunctions.net/SIMF'
        req = urequests.get(url, headers=headers)
        
        if req.status_code == 200:
            JSONdeviceIDResponse = json.loads(req.text)
            ConnectedIP = get_json_key(JSONdeviceIDResponse, 'ConnectedIP')
